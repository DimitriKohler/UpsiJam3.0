...
public class MageSplitController : MonoBehaviour
{
    ...
    [SerializeField] private GameObject mageSpawn;
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("Bullet") && !cdHurt)
        {
            ...
            if (lives <= 0 && !isDead && !hasSpawned)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    Instantiate(mageSpawn, transform.position, Quaternion.identity);
                }
